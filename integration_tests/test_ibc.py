...
def test_precompiles_ibc(ibc):
    ...
    def assert_transfer(timeout, diff):
        params = Params(port_id, channel_id, src_denom, dst_denom, ratio, timeout)
        ...
        next_ack_seq = contract.caller.nativeQueryNextSeq(port_id, channel_id)
        assert last_ack_seq + diff == next_ack_seq, "expect ack diff: {diff}"
        for _ in range(max_retry):
            has = contract.caller.nativeHasCommit(port_id, channel_id, last_ack_seq)
            next_ack_seq = contract.caller.nativeQueryNextSeq(port_id, channel_id)
